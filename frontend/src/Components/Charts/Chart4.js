...
function Chart4() {
    ...
    useEffect(() => {
        ...
        const afterFourDays = new Date(currentDate);
        afterFourDays.setDate(afterFourDays.getDate() + 4);

        const formattedCurrentDate = currentDate.toISOString().split("T")[0];
        const formattedAfterFourDays = afterFourDays.toISOString().split("T")[0];

        axios
            .get(`${api.base}forecast?q=${defaultCity}&units=metric&APPID=${api.key}`)
            .then((res) => {
                const afterFourDaysData = res.data.list.filter(
                    (data) => data.dt_txt.split(" ")[0] === formattedAfterFourDays
                );
                setForecast(afterFourDaysData);
            })
            .catch((error) => {
