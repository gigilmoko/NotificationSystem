...
function Chart2() {
    ...
    useEffect(() => {
        ...
        const formattedAfterTwoDays = afterTwoDays.toISOString().split("T")[0];

        axios
            .get(`${api.base}forecast?q=${defaultCity}&units=metric&APPID=${api.key}`)
            .then((res) => {
                const afterTwoDaysData = res.data.list.filter(
                    (data) => data.dt_txt.split(" ")[0] === formattedAfterTwoDays
                );
                setForecast(afterTwoDaysData);
            })
            .catch((error) => {
