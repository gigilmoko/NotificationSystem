...
function Chart5() {
    ...
    useEffect(() => {
        ...
        const afterFiveDays = new Date(currentDate);
        afterFiveDays.setDate(afterFiveDays.getDate() + 5);

        const formattedCurrentDate = currentDate.toISOString().split("T")[0];
        const formattedAfterFiveDays = afterFiveDays.toISOString().split("T")[0];

        axios
            .get(`${api.base}forecast?q=${defaultCity}&units=metric&APPID=${api.key}`)
            .then((res) => {
                const afterFiveDaysData = res.data.list.filter(
                    (data) => data.dt_txt.split(" ")[0] === formattedAfterFiveDays
                );
                setForecast(afterFiveDaysData);
            })
            .catch((error) => {
